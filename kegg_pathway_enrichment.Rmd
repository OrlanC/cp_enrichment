...
#install.packages("R.utils") #Uncomment to install for the first time and reinstall if required (in case of error).
...
data = read.table("input_file_2.txt", header = TRUE, sep="\t",  quote= "", row.names=1, comment.char="")
head(data)
id<-rownames(data)
...
enKEGG<-enrichKEGG(knum, organism = 'ko', minGSSize = 1,keyType = "kegg", pvalueCutoff = 0.05,pAdjustMethod = "BH", qvalueCutoff = 1)

write.table(enKEGG, file=paste(outpathcount,"KEGG.txt", sep = ""), row.names=TRUE, col.names=NA, quote=FALSE, sep="\t") #export only significant results, use enKEGG@result to get all results (significant and not significant results)
...
![KEGG Native Enriched Pathway Plot](results/map00564.png)
